function [avgSame, sdSame, avgOpp, sdOpp, avgFull, sdFull] = relativeTiltCalc(pnum)
%
...
matFileName = sprintf('%s_RT_age_og.mat', folderNum);
...
% presentation) (different columns in pilot & ageing)
for i = 1 : nTrials
    if RTdat(i,4) < 0
...
%hemi = RTdat(:,3) > 0;
%hemiRTs = RTdat(hemi,8)

%avgHemi = mean(hemiRTs);
%sdHemi = std(hemiRTs);
